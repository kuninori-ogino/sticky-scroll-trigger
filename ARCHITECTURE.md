...
`inner` holds the effective pin range, and `outer` (`contain: layout`) isolates its overflow. Every `refresh()` sizes `outer` to `trigger`'s own margin box before anything else is measured (`#reservePinSpace`), so the page keeps the height and the layout it had before the pin was registered. Horizontal sizing is unchanged; `trigger`'s own `width`/`flex` rules still apply.

`#reservePinSpace` measures that margin box instead of computing it: it clears `inner`'s range and `outer`'s own height, then reads back the natural height `outer` collapses to. Because `contain: layout` makes `outer` a formatting context, `trigger`'s margins land inside it rather than collapsing out, so that natural height is the margin box already, resolved by the browser through whatever `box-sizing`, borders, floats or line boxes `trigger` happens to have. Reading it off the computed style (`measureUsedHeight`) keeps the fractional part that `offsetHeight` rounds away, and keeps out a transform that `getBoundingClientRect` would include and no other measurement here does.

Margin collapsing is the one thing the reserved height can't stand in for: `trigger`'s vertical margins stop at `outer` instead of collapsing with its siblings', and no sizing of `outer` can fix that, since the collapsing partner is outside it. A 30px `margin-bottom` above a `trigger` with a 20px `margin-top` occupies 30px unwrapped and 50px wrapped. That leftover shift is why `#unwrapPinLayer` still schedules a rebuild on kill. `inner` carries no `overflow` or `contain` of its own on purpose: either would make it a formatting context too, and `trigger`'s top margin would then push it down inside the reserved box rather than collapsing out to `outer`.
