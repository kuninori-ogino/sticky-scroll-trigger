...
A `trigger` shorter than the viewport that also sits within a viewport height of the document top makes that default resolve to a negative freeze-window start, so part of the rise plays before scroll 0. The freeze window is not clamped here; see [Position syntax](#position-syntax) for why and how to avoid it.
...
The table above is the whole accepted grammar. GSAP's `clamp()` wrapper (`start: 'clamp(top center)'`, which holds the resolved position inside the scroller's own range) is not part of it and throws. Nothing clamps the freeze window either, so a clause that resolves past the top of the document keeps its negative value: a Scene layer sitting at the very start of the container with `start: 'top bottom'` begins one viewport height before scroll 0, its wrapper torn that far down the flow and its dwell cut to match, so a scrubbed tween is already part-done at load. [`createOverlapScroll`](#createoverlapscrolloptions)'s `'bottom bottom'` default reaches this with no `start` passed when the covered `trigger` sits within a viewport height of the document top and is shorter than the viewport (`freezeStart` is `triggerTop - (viewportHeight - triggerHeight)`), so part of the rise is already showing at scroll 0. To avoid it, give `trigger` at least a viewport's height, or move it further down the page. There is no opt-out here: `clamp()` is rejected, and a cover layer's sticky offset does not reference the freeze window's start, so clamping the window would not move it. `end` is the exception, already clamped to `start`.
...
GSAP treats a value as an absolute scroll position rather than a clause whenever it's just a number: a plain JS number, or a string `Number()` converts on its own with no keyword, second token, or `%`/`px` suffix, `'-50'` and `'1e3'` included. It ignores the reference element entirely in that case (`ScrollTrigger.js`'s `_parsePosition`: `isNaN(value) || (value = +value)`). This module follows the same rule for `start` and `end`, and for [`createStickyPin`](#createstickypinoptions)'s `end`, [`createResolvedTrigger`](#createresolvedtriggeroptions)'s `start`/`end`, and [`resolveScrollPosition`](#resolvescrollpositionelement-position)'s `position`.
